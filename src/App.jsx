...
export default function App() {
  ...
  return <>
    ...
    <main id="main">
      ...
      <section className="manifesto"><p className="section-label" data-reveal>[ 01 / EL CONGRESO ]</p><div data-reveal><h2>Una mirada al <i>futuro de sistemas.</i></h2><p className="lead">Un espacio académico e innovador que se llevará a cabo el día 17 de octubre del presente año. El evento se realizará en FUNPARK (Santa Lucía Los Ocotes, Zona 25, Ciudad de Guatemala). Este evento reunirá a estudiantes, catedráticos y profesionales de las ciencias de la computación y sistemas con el objetivo de intercambiar conocimientos sobre las últimas tendencias, innovaciones y herramientas tecnológicas que están transformando el sector.</p></div><div className="event-highlights" data-reveal aria-label="Datos relevantes del evento"><article><strong>+1000</strong><span>Asistentes<br />presenciales</span></article><article><strong>+30</strong><span>Speakers<br />líderes</span></article><article><strong>+30</strong><span>Conferencias<br />especializadas</span></article><article><strong>+3</strong><span>Paneles<br />empresariales</span></article><article><strong>+6</strong><span>Talleres<br />prácticos</span></article><article><strong>+</strong><span>Salas de<br />networking</span></article></div></section>
      <section id="ponentes" className="speakers section-dark"><div className="section-heading" data-reveal><p className="section-label">[ 03 / VOCES ]</p><h2>Personas que <i>crean</i> futuro.</h2><p>Conocimiento, experiencias reales y comunidad tecnológica en un mismo lugar.</p></div><div className="speaker-grid" data-reveal>{speakers.map(([color, portrait, initials, name, role, city]) => <article className={`speaker ${color}`} key={name}><div className={`portrait ${portrait}`}><span>{initials}</span></div><div className="speaker-info"><p>{name}</p><span>{role}<br />{city}</span></div></article>)}</div><a className="text-link" data-reveal href="#programa">Conoce al resto de la comunidad <span>→</span></a></section>
      <section className="sponsors"><div className="sponsors-heading" data-reveal><p className="section-label">[ 04 / ALIANZAS ]</p><h2>Patrocinadores que<br /><i>impulsan</i> el futuro.</h2></div><div className="sponsor-grid" data-reveal><article className="sponsor-card"><img src="https://desarrolloweb.s3.us-east-1.amazonaws.com/ByteConUMG/Claro_logo_(2017).svg" alt="Claro" /></article><article className="sponsor-card"><img src="https://desarrolloweb.s3.us-east-1.amazonaws.com/ByteConUMG/FunPark_logo.png" alt="FunPark" /></article></div></section>
      <section id="programa" className="schedule"><header className="schedule-header" data-reveal><p className="section-label">[ 04 / PROGRAMA ]</p><h2>Programa del <i>evento.</i></h2><div><time dateTime="2026-10-17">17 de octubre de 2026</time><a className="schedule-calendar" href="#entradas">Añadir al calendario</a></div></header><div className="schedule-list" data-reveal><p className="schedule-break">Bienvenida · 09:30</p>{talks.map(([time, title, place, type], index) => <article className={`talk${index === 0 ? ' featured-talk' : ''}`} key={time}><time>{time}</time><div><p className="talk-type">{type}</p><h3>{title}</h3><p>{place}</p></div></article>)}<p className="schedule-break">Pausa de café · 14:00</p></div></section>
